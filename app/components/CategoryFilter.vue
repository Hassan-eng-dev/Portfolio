...
<template>
  <div class="admin-scroll -mx-6 flex gap-2 overflow-x-auto px-6 pb-2 sm:mx-0 sm:flex-wrap sm:px-0" role="group" :aria-label="t('categoryFilter.ariaLabel')">
    <button
      type="button"
    ...
    >
      ...
      />
      <span class="relative">{{ t('categoryFilter.allWork') }}</span>
    </button>
    <button
    ...
    >
      <motion.div
        ...
        :transition="{ type: 'spring', stiffness: 380, damping: 30 }"
      />
      <span class="relative">{{ categoryLabel(category) }}</span>
    </button>
  </div>
</template>
